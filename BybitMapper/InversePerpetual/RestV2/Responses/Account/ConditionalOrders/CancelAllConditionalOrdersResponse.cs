...
namespace BybitMapper.InversePerpetual.RestV2.Responses.Account.ConditionalOrders
{
    ...
    {
        [JsonConstructor]
        public CancelAllConditionalOrdersResponse(int retCode, string retMessage, string extCode, string extInfo, double timestamp, double rateLimitStatus, double rateLimitResetMs, double rateLimit, IReadOnlyList<CancelAllConditionalOrdersData> result)
        {
            RetCode = retCode;
        ...
        [JsonProperty("result")]
        public IReadOnlyList<CancelAllConditionalOrdersData> Result { get; }
    }
}
