...
{
    /// <summary>
    /// USDT Perpetual класс для парсинга response QueryConditionalOrder
    /// </summary>
    public sealed class QueryConditionalOrderResponse
    {
        [JsonConstructor]
        public QueryConditionalOrderResponse(int retCode, string retMessage, string extCode, string extInfo, double timestamp,
            double rateLimitStatus, double rateLimitResetMs, double rateLimit, IReadOnlyList<DataGetConditionalOrderData> result)
        {
            RetCode = retCode;
        ...
        [JsonProperty("result")]
        public IReadOnlyList<DataGetConditionalOrderData> Result { get; }
    }
}
